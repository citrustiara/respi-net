...
estymator konsensusu harmonicznych HB100 odtworzył zadany rytm
\SI{0.20}{Hz} w~zakresie \SIrange{0.190}{0.204}{Hz}, lecz jednokanałowy
przebieg nie zachowywał stabilnie kierunku wdech--wydech; A121 umożliwiał taką
analizę na podstawie fazy I/Q. Przeprowadzono również
...
ilościową nadal należy sprawdzić histogram $\Delta t$, liczbę utraconych próbek
i~rzeczywistą liczbę próbek na sekundę dla każdego urządzenia.
...
FFT. Przyjęto stałą konwencję: przejście od maksimum do minimum oznacza
wdech, a~od minimum do następnego maksimum --- wydech. Nie wykonano ręcznych
...
rysunkach~\ref{fig:hb100-a121-surowe} i~\ref{fig:hb100-a121-oddech}.

Ponieważ znak fazy A121 i~napięcia HB100 jest umowny, dla każdego toru wybrano
jedną polaryzację maksymalizującą zgodność z~pierwszym blokiem sterowanym.
Nie zmieniano jej już w~drugim bloku. Jest to celowo łagodny test HB100:
pozwala na jednorazową kalibrację znaku, lecz sprawdza, czy mapowanie pozostaje
stabilne po wstrzymaniu oddechu.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{hb100_a121_surowe.png}
    \caption[Surowe przebiegi A121 i HB100]{Równoczesne przebiegi przed
    cyfrową filtracją oddechową. Dla A121 pokazano rozplecioną i~pozbawioną
    trendu fazę IQ, a~dla HB100 napięcie ADC na wyjściu analogowego front-endu.
    Ze względu na różne jednostki oba sygnały znormalizowano osobno zakresem
    5.--95. percentyla; porównywać można czas i~kształt, lecz nie wysokość
    krzywych. Niebieskie tło oznacza wdech, pomarańczowe wydech, a~szare
    wstrzymanie po korekcie opóźnienia $\delta$.}
    \label{fig:hb100-a121-surowe}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{hb100_a121_oddech.png}
    \caption[Składowe oddechowe A121 i HB100]{Składowe oddechowe obu radarów
    na tle skorygowanych komend. Linia kropkowana jest zadanym trójkątnym
    cyklem, niebieska --- przemieszczeniem z~A121, a~pomarańczowa --- napięciem
    HB100. Polaryzację obu torów ustalono tylko na pierwszym bloku i~zachowano
    w~drugim. $r_{\mathrm{HB},2}$ nad panelem jest korelacją HB100 z~komendami
    w~drugim bloku; ujemna wartość oznacza odwrócenie relacji po jednorazowej
    kalibracji. W~każdej próbie oba sygnały wyraźnie zanikają podczas
    wstrzymania, lecz HB100 często wykonuje dwa lub trzy oscylacje na jeden
    cykl klatki piersiowej.}
    \label{fig:hb100-a121-oddech}
\end{figure}

\section{Estymacja rytmu z jednokanałowego HB100}
...
przemieszczenie klatki. Jednokanałowe napięcie IF HB100 nie jest natomiast
ani przemieszczeniem, ani prędkością. Dla modelu~\eqref{eq:cw-baseband}
różniczkowanie daje
\begin{equation}
...
w~punktach zerowych. Dlatego pochodna napięcia IF nie jest estymatą prędkości,
a~całkowanie napięcia ani jego pochodnej nie odtwarza przemieszczenia. Dopiero
para kanałów I/Q pozwoliłaby odzyskać ciągłą fazę i~jej znak.
...
widma. Wynik jest publikowany tylko, gdy co najmniej dwie składowe wnoszą po
...
\begin{figure}[H]
    ...
    \SI{150}{cm}. Górny panel pokazuje przemieszczenie wyznaczone z~fazy IQ
    ...
    zadanego rytmu \SI{0.20}{Hz}. Nie nadaje to sygnałowi HB100 kierunku
    wdech--wydech.}
...
\eqref{eq:hb100-harmonic-consensus}. Nie zastępuje ona powyższej oceny
kierunku: jej celem jest wyłącznie ustalenie, czy z~nieliniowego napięcia
HB100 można odzyskać rytm kontrolowanego ruchu mimo dominacji harmonicznych.

\begin{table}[H]
\centering
\caption{Sprawdzenie estymatora konsensusu harmonicznych HB100 w~14
sterowanych blokach po \SIrange{29.8}{35.0}{s}. Rytm zadany wynosił
\SI{0.20}{Hz}; oddechy były pełne i~pogłębione.}
...
Estymata konsensusu harmonicznych $\hat f$ & \SIrange{0.190}{0.204}{Hz} \\
Mediana $|\hat f-\SI{0.20}{Hz}|$ & \SI{0.001}{Hz} (0,06~oddechu/min) \\
Okna spełniające wszystkie warunki jakości & 14 / 14 \\
Retest \SI{150}{cm}: blok 1 / blok 2 & \SI{0.190}{Hz} / \SI{0.202}{Hz} \\
\bottomrule
\end{tabular}
\end{table}

W~13 z~14 bloków odchylenie nie przekroczyło \SI{0.004}{Hz}; największe,
\SI{0.010}{Hz}, wystąpiło w~pierwszym bloku pojedynczego retestu
\SI{150}{cm}. Wynik pokazuje użyteczność sumowania harmonicznych dla
powtarzalnego, silnego ruchu, ale nie jest niezależną walidacją kliniczną:
ten sam, jeden badany wykonywał narzucone oddechy 12~oddechów/min. W~szczególności
nie dowodzi poprawnej estymacji płytszego oddechu naturalnego, zmiennego rytmu
ani oddechu przez kołdrę.
...
\begin{figure}[H]
    ...
    Lewy dolny panel pokazuje poziom składowej \SI{0.20}{Hz} względem
...
jednorazowej kalibracji znaku wyniosła $-0{,}299$, a~zgodność kierunku
wdech--wydech 36,6\%. Jednocześnie konsensus harmoniczny wskazał
\SI{0.190}{Hz} i~\SI{0.202}{Hz}, czyli rytm bliski zadanym \SI{0.20}{Hz}
(rys.~\ref{fig:hb100-a121-150cm}). W~tym samym nagraniu A121 z~płaską
pokrywą osiągnął $r=0{,}910$ i~96,7\% zgodności kierunku. Wynik wskazuje więc
praktyczną granicę użycia HB100 do rozróżniania faz w~tej geometrii, przy
jednoczesnej możliwości warunkowego oszacowania samej częstości. Jest to jednak
tylko jeden retest; nie wyznacza ogólnego zasięgu żadnego z~radarów,
a~zwłaszcza A121 z~soczewką skupiającą.
...
\paragraph{HB100 nie zachowywał wiarygodnie faz wdech--wydech.} Dla A121
korelacja drugiego bloku z~komendami wynosiła 0,894--0,957, a~zgodność
kierunku 95,9--99,3\%. Dla HB100 wartości wynosiły odpowiednio od $-0{,}543$
do 0,438 oraz 36,5--63,7\%, czyli pozostawały blisko poziomu losowego 50\%.
W~jednej próbie na \SI{60}{cm} relacja była umiarkowanie dodatnia w~pierwszym
bloku, a~po wstrzymaniu stała się wyraźnie ujemna mimo niezmienionej
polaryzacji. Jednorazowa kalibracja znaku nie wystarcza zatem do rozróżniania
wdechu i~wydechu.
...
sterowanego oddechu estymator konsensusu harmonicznych pozwolił także
warunkowo wyznaczyć częstość, łącząc kandydatów $f$, $2f$ i~$3f$. Kierunku
wdech--wydech nie udało się jednak zachować po jednorazowej kalibracji znaku.
...
który udostępnia bramki odległościowe i~zespolony sygnał I/Q. Estymator HB100
może służyć jedynie jako ostrożny wskaźnik rytmu w~stabilnym, wystarczająco
silnym fragmencie ruchu, z~jawnym odrzuceniem wyniku niejednoznacznego.
...
\begin{itemize}
  ...
  w~14 blokach rytm \SIrange{0.190}{0.204}{Hz} przy zadanych
  \SI{0.20}{Hz}, ale nie odtwarza przemieszczenia ani faz cyklu. A121 osiągał
  w~tym samym teście 95,9--99,3\% opisowej zgodności kierunku, dlatego to jego
  faza I/Q jest właściwym wejściem do analizy faz cyklu. Pojedynczy retest
  \SI{150}{cm} podtrzymał ten wniosek: HB100 wykazał zanik przy wstrzymaniu
  i~rytm po złożeniu harmonicznych, lecz nie kierunek fazy, podczas gdy A121
  z~płaską pokrywą zachował 96,7\% zgodności kierunku.
