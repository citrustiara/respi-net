...
\begin{figure}[H]
    ...
    sygnału w~funkcji odległości dla wszystkich 36~nagrań. Soczewki skupiające
    dają 3--10-krotnie wyższy pik od klatki piersiowej; przy \SI{100}{cm} pik
    płaskiej pokrywy jest szeroki i~ledwo odróżnialny od tła.}
    \label{fig:profile}
\end{figure}
...
\begin{figure}[H]
    ...
    z~bramek klatki piersiowej. Każde nagranie wykazuje tę samą strukturę:
    dominujący pik oddechowy w~zacienionym paśmie, harmoniczne oraz szumowe
    tło. Wybór soczewki niemal nie zmienia odległości piku od tła.}
    \label{fig:widma}
\end{figure}
...
\begin{figure}[H]
    ...
    36~nagrań (punkty wypełnione --- folia, puste --- brak folii; lewa/prawa
    podkolumna --- \SI{50}{cm}/\SI{100}{cm}). Amplituda i~stabilność śledzenia
    różnicują soczewki o~rząd wielkości; SNR i~wyrazistość piku --- nie.}
    \label{fig:metryki}
\end{figure}
...
\begin{figure}[H]
    ...
    folia~$-$~brak folii. Amplituda oscyluje wokół zera; spadek szumu
    ruchowego jest jednorodny dla wszystkich soczewek --- sygnatura efektu
    sesji, a~nie efektu reflektora.}
    \label{fig:delty}
\end{figure}
...
\begin{figure}[H]
    ...
    przemieszczenia klatki piersiowej dla każdego z~12~warunków. Wszystkie
    przebiegi są czytelne; przebiegi z~płaską pokrywą mają wyraźnie mniejszą
    amplitudę.}
    \label{fig:przebiegi}
\end{figure}
...
\todo{Uzupełnić wyniki retestu na dystansie \SI{2}{m}, wykonanego wyłącznie
z~soczewką hiperboliczną w~układzie $2\times2$: naturalny kąt padania około
\SI{30}{\degree} względem normalnej oraz padanie prawie prostopadłe, każde
bez folii i~z~folią. Warunki przeplatać w~jednej sesji i~wykonać po trzy
powtórzenia.}
...
z~zegarka Garmin~Fenix~7 traktowanego jako referencja.

\section{Metodyka pomiarów}
Zarejestrowano serię nagrań całonocnych. Radar umieszczono przy łóżku,
skierowany na klatkę piersiową śpiącego. Równolegle badany nosił zegarek
Garmin~Fenix~7, z~którego eksportowano dane w~formacie FIT (tętno, oddech,
fazy snu). Trzy noce analizowane szczegółowo różniły się soczewką:
\begin{itemize}
...
\todo{Uzupełnić: dokładny opis stanowiska (wysokość, kąt, odległość),
zdjęcia stanowiska nocnego, procedura synchronizacji czasu radaru z~zegarkiem.}
...
referencyjnym --- opracowano algorytm, który \emph{z~grubsza odtwarza
...
Sen głęboki [min]      & 78,5 & 57,0 & 82,0 \\
Sen lekki [min]        & 212,5 & 222,0 & 159,0 \\
REM [min]              & 73,5 & 47,0 & 116,0 \\
Czuwanie [min]         & 24,0 & 40,0 & 24,0 \\
\midrule
Sen głęboki [\% snu]   & 21,5 & 17,5 & 23,0 \\
REM [\% snu]           & 20,2 & 14,4 & 32,5 \\
\midrule
...
za odniesienie, radar raz \emph{zawyża} sen głęboki (o~21~min względem FIT),
a~raz \emph{zaniża} (o~3,5~min względem aplikacji) --- przy dokładnie tych
...
\begin{table}[H]
\centering
\caption{Podsumowanie snu wyznaczone wyłącznie z~radaru.}
\label{tab:sensummary}
\small
\begin{tabular}{@{}lrrrrl@{}}
\toprule
Noc & Ocena & Sen całk. [min] & Efektywność [\%] & Spocz. HR [bpm] & Jakość \\
\midrule
1 (plano-convex) & 74 & 364,5 & 93,8 & 52,7 & Przeciętna \\
2 (FZP)          & 71 & 342,5 & 95,0 & 51,9 & Przeciętna \\
3 (płaska)       & 49 & 154,0 & 85,8 & 50,8 & Słaba \\
\bottomrule
\end{tabular}
\end{table}
...
spoczynkowe pozostało spójne między nocami (nisko w~przedziale 50~bpm).
...
soczewki od wpływu ułożenia ciała; jest to interpretacja, a~nie wykazany
związek przyczynowy.
...
obciążenie nie przekracza pół oddechu na minutę dla żadnej z~nocy. Jest to
najmocniejszy i~najbardziej stabilny wynik całego badania nocnego.
...
82~minuty --- czyli \emph{więcej} niż radar (78,5~min). Kierunek rozbieżności
...
Istotna obserwacja diagnostyczna dotyczy natomiast \emph{struktury} hipnogramu:
w~obu nocach klasyfikator radarowy wykazał bezpośrednie przejścia
REM~$\rightarrow$~sen głęboki (po dwa na noc), podczas gdy Garmin nie wykazał
żadnego takiego przejścia. Takie przejście jest w~fizjologii snu nietypowe
(sen głęboki poprzedza zwykle REM w~ramach cyklu, a~wyjście z~REM prowadzi
przez sen lekki lub wybudzenie), co wskazuje na słabość samej heurystyki
regułowej --- a~nie na błąd śledzenia tętna i~oddechu, które w~tych samych
oknach zgadzało się z~zegarkiem.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{sen_noc1_garmin_overlay.png}
    \caption[Noc 1 --- nałożenie danych radaru i Garmina]{Noc~1 (soczewka
    plano-convex). Tętno i~oddech z~radaru oraz heurystyczne fazy snu
    nałożone na dane referencyjne z~Garmina.}
    \label{fig:noc1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{sen_noc2_garmin_overlay.png}
    \caption[Noc 2 --- nałożenie danych radaru i Garmina]{Noc~2 (soczewka
    FZP). Noc o~bardzo wysokiej amplitudzie dominującego odbicia, przy czym
    najsilniejszy reflektor zwykle \emph{nie} znajdował się na śledzonej
    odległości celu.}
    \label{fig:noc2}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{sen_noc3_garmin_overlay.png}
    \caption[Noc 3 --- nałożenie danych radaru i Garmina]{Noc~3 (płaska
    pokrywa). Nagranie zostało przerwane, ale zgodność tętna z~Garminem była
    najlepsza z~trzech nocy.}
    \label{fig:noc3}
\end{figure}
...
tułowia. Taki mechanizm jest zgodny z~najlepszą zgodnością tętna w~nocy
z~płaską pokrywą oraz z~problemem piku od krawędzi łóżka, ale wymaga osobnego,
kontrolowanego testu kąta lub przesunięcia poprzecznego.

Najważniejsze ograniczenia badania to jedna osoba, tylko trzy noce, inna
soczewka każdej nocy i~brak polisomnografii. Garmin jest konsumenckim źródłem
porównania, a~nie złotym standardem. Z~tego powodu wyników nie należy
interpretować jako rankingu soczewek ani walidacji klinicznej algorytmu snu.
