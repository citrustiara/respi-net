...
i podgądu na żywo oraz zestaw narzędzi do analizy offline. Przeprowadzono
...
\toprule
Okres & Etap & Zakres prac \\
\midrule
III 2026 & Start & Pierwsza akwizycja, uruchomienie toru radarowego HB100,
podstawowe wykresy i FFT. \\
IV--V 2026 & Tor IMU & Akwizycja I2C z LSM6DS3, analiza PCA, filtry
Butterwortha; wyodrębnienie rytmu oddechu i tętna. \\
18 V 2026 & \emph{Zmiana kierunku} & Przeniesienie głównego nacisku
z~czujnika IMU na tor radarowy. \\
31 V 2026 & Integracja A121 & Wsparcie dla radaru Acconeer~A121, schemat
front-endu analogowego, prototyp aplikacji. \\
1 VI 2026 & Parametry witalne & Poprawki algorytmu estymacji oddechu
i~tętna (bramkowanie odległościowe, miary pewności). \\
VI 2026 & Eksperymenty & Pomiary całonocne, aplikacja na iPhone, rozbudowa
bazy nagrań. \\
VII 2026 & Eksperyment sterowany & Automatyczna procedura badania soczewek
...
$R_g = \SI{10}{k\ohm}$, co daje wzmocnienie około $11\times$ na stopień
i~w~przybliżeniu $121\times$ łącznie. Kondensator \SI{22}{nF} w~pętli
sprzężenia zwrotnego ogranicza pasmo od góry. Wejścia sprzężone pojemnościowo
...
\begin{figure}[H]
    ...
    dwa stopnie wzmacniające MCP6002 ($\approx 121\times$), filtracja
    pasmowa, wirtualna masa \SI{1.65}{V} i separowane zasilanie analogowe
    MCP1700.}
...
\miejsce{Do uzupełnienia: obliczenia projektowe (dobór częstotliwości
granicznych filtru pod pasmo oddechu 0,1--0,5~Hz i tętna 0,7--2,0~Hz),
charakterystyka częstotliwościowa układu (symulacja i~pomiar), analiza szumów
oraz pomiary oscyloskopem.}
...
\SI{2}{ms}, czyli nominalnie \SI{500}{Hz}; jest to duży nadmiar względem pasm
oddechu i~tętna, ale ułatwia późniejsze filtrowanie i~ocenę szumu toru.

Każdy wiersz transmisji przy \SI{921600}{baud} zawiera znacznik czasu z~zegara
...
kontrolą spójności, ale sama nie czyni IMU wzorcem dla radaru. Ocena dotyczy
tego, czy sygnał inercyjny jest dostatecznie czytelny w~pozycji siedzącej
i~leżącej oraz czy prosty tor z~telefonem może zastąpić osobny moduł sprzętowy.
...
ani przesunięcia podczas pomiaru. To założenie wymaga sprawdzenia osobno dla
...
Podstawowy eksperyment obejmuje dla obu urządzeń pozycję siedzącą i~leżącą,
oddech swobodny, oddech narzucony metronomem oraz krótki odcinek wstrzymania
oddechu. Porównywane powinny być: dominująca częstotliwość, SNR widmowy,
...
\paragraph{Uczciwe zastrzeżenie.} Jak wynika z~eksperymentu opisanego
...
\midrule
\texttt{b9f49e8} (31.05) & \SIrange{0.08}{0.70}{Hz} & \SIrange{0.65}{3.00}{Hz} & pierwsza wersja \\
\texttt{7924d95} (31.05) & \SIrange{0.10}{0.70}{Hz} & \SIrange{0.65}{3.00}{Hz} & korekta $f_s$, tryb live \\
\texttt{9d2130a} (01.06) & \SIrange{0.10}{1.00}{Hz} & \SIrange{0.65}{3.00}{Hz} & procesor Acconeera \\
\texttt{bf82a08} (11.06) & \SIrange{0.10}{0.50}{Hz} & \SIrange{0.70}{2.00}{Hz} & wersja obecna \\
\bottomrule
\end{tabular}
\end{table}
...
nagraniami bez folii, w~dwóch różnych sesjach (13 i~14~lipca~2026).
...
płycej podczas sesji nocnej 13~lipca niż wieczorem 14~lipca. Prawdziwego
...
Retest wykonano 15~lipca~2026 wyłącznie z~soczewką hiperboliczną, ponieważ
...
Równolegle badany nosił zegarek Garmin~Fenix~7, z~którego --- jeżeli eksport
zachował się dla danej nocy --- odczytywano tętno, częstość oddechu i~fazy snu
w~formacie FIT. Pierwsze trzy nagrania służyły pilotażowemu porównaniu
przesłon:
\begin{itemize}
  \item noc~1 --- soczewka plano-convex,
  \item noc~2 --- soczewka FZP,
  \item noc~3 --- płaska pokrywa (efektywnie brak soczewki); nagranie zostało
  przerwane, przez co jest krótsze.
\end{itemize}
...
W~trzech późniejszych, pełnych nagraniach z~13, 15 i~16~czerwca zastosowano
...
\paragraph{Status wyniku: estymacja, nie pomiar.} Trzeba postawić sprawę
otwarcie: \textbf{wyznaczonych faz snu nie da się w~tej pracy zweryfikować}.
...
\paragraph{Czym ta liczba jest, a~czym nie jest.} Warto zacząć od zastrzeżenia,
które rzadko pada w~materiałach producentów: \emph{ocena snu} (\emph{sleep
score}) w~skali 0--100 jest \textbf{metryką wymyśloną przez producentów
urządzeń}, a~nie wielkością fizjologiczną ani kliniczną. Nie odpowiada jej
żadna jednostka, żadne badanie referencyjne i~żadna definicja w~medycynie snu
--- polisomnografia nie zwraca ,,74~punktów''. Każdy producent (Garmin, Fitbit,
Oura, Withings) liczy ją inaczej i~z~innych składowych, a~konkretne wzory są
tajemnicą handlową; ta sama noc oceniona dwoma urządzeniami potrafi dać
istotnie różne wyniki. \textbf{Sama w~sobie liczba ta nic nie znaczy} --- jest
co najwyżej wygodnym skrótem, który agreguje kilka mierzalnych cech nocy
(długość, ciągłość, proporcje faz, spokój, parametry witalne) w~jedną wartość
porównywalną \emph{ze samą sobą} z~nocy na noc.
...
konsumenckim --- opracowano algorytm, który \emph{z~grubsza odtwarza
...
--- ,,wystarczająco dobra'' wartość cechy daje komplet punktów, a~pogorszenie
karane jest liniowo.
...
74). Innymi słowy: \emph{ocena mierzy głównie to, ile się spało}, a~pozostałe
składowe jedynie ją modulują --- co jest zarazem trafną obserwacją o~tym, jak
działają oceny snu w~urządzeniach komercyjnych.
...
\paragraph{Czy to rzeczywiście ,,matchuje'' Garmina?} Dla nocy~1 udało się
...
Zgodność samej oceny jest uderzająca: \textbf{74~punkty w~obu przypadkach}
i~identyczna etykieta jakości, przy niezależnie wyznaczonym czasie snu
różniącym się o~7~minut. Zgadza się też tętno spoczynkowe --- \SI{52.7}{bpm}
z~radaru wobec \SI{51}{bpm} z~zegarka --- co zamyka pytanie postawione przy
opisie estymatora tętna spoczynkowego.
...
\paragraph{Dwa Garminy, jedna noc.} Tabela~\ref{tab:score-garmin} ujawnia
jednak coś, co jest w~tej pracy ważniejsze od samej zgodności oceny:
...
Wniosek jest niewygodny, ale nieunikniony: \emph{Garmin nie jest wzorcem,
skoro nie zgadza się sam ze sobą}. Zależnie od tego, który eksport przyjmiemy
za odniesienie, radar raz \emph{zawyża} sen głęboki (o~13,5~min względem FIT),
a~raz \emph{zaniża} (o~11,5~min względem aplikacji) --- przy dokładnie tych
samych danych radarowych. To najmocniejszy dostępny w~tej pracy argument za
tym, że rozbieżności faz snu z~sekcji~\ref{sec:fazy-wyniki} nie wolno
interpretować jako ,,błędu radaru''.

Sama zgodność oceny (74 vs 74) też nie jest dowodem poprawności --- i~warto
rozumieć, \emph{dlaczego} wyszła tak dobrze. Ocena jest zdominowana przez czas
snu (wzór~\eqref{eq:duration-cap}), a~czas snu obie metody mierzą podobnie, bo
obie wykrywają to samo: brak ruchu i~spadek tętna. Rozjazd ujawnia się dopiero
tam, gdzie czas snu nie decyduje --- w~proporcjach faz. \textbf{Zgodne oceny
przy niezgodnych fazach} to dokładna ilustracja tezy tej sekcji: ocena snu
jest agregatem o~małej rozdzielczości informacyjnej. Jedna noc to zresztą
anegdota, a~nie walidacja.
...
są porównywalne między producentami. Eksport \texttt{Sleep.csv} udało się
odzyskać tylko dla nocy~1 --- dla pozostałych nocy warto go dociągnąć
z~Garmin~Connect i~powtórzyć tabelę~\ref{tab:score-garmin}, żeby sprawdzić,
czy rozjazd FIT~vs~Connect powtarza się co noc.}

\section{Wyniki: podsumowanie radarowe}

\begin{table}[H]
\centering
\caption{Podsumowanie snu wyznaczone wyłącznie z~radaru. Trzy ostatnie
wiersze dotyczą pełnych nocy z~płaską pokrywą i~czujnikiem nad łóżkiem.}
...
\midrule
1 (plano-convex, krawędź) & 74 & 364,5 & 93,8 & 52,7 & Przeciętna \\
2 (FZP, krawędź)          & 71 & 342,5 & 95,0 & 51,9 & Przeciętna \\
3 (płaska, zapis przerwany)& 49 & 154,0 & 85,8 & 50,8 & Słaba \\
13.06 (płaska, nad łóżkiem)& 93 & 485,0 & 97,1 & 49,7 & Doskonała \\
15.06 (płaska, nad łóżkiem)& 78 & 454,5 & 89,4 & 51,1 & Przeciętna \\
16.06 (płaska, nad łóżkiem)& 91 & 551,0 & 97,6 & 50,5 & Doskonała \\
\bottomrule
\end{tabular}%
}
\end{table}

Noc~3 wypadła znacznie gorzej głównie dlatego, że nagranie zostało przerwane
i~jest krótkie --- ocena została ograniczona przez czas snu. Oszacowane tętno
spoczynkowe pozostało spójne między nocami (około 50--53~bpm). Trzy późniejsze
pełne noce pokazują, że w~ustawieniu nad łóżkiem z~płaską pokrywą można
powtarzalnie uzyskać wielogodzinne, w~dużej części ciągłe trendy radarowe.
Nie potwierdza to jednak poprawności etykiet faz ani samo w~sobie nie dowodzi
przewagi tej przesłony: punktowa ocena rośnie przede wszystkim z~długością
i~ciągłością zapisu.
...
\midrule
1 (plano-convex) & 55,5 & 57,4 & $-1{,}9$ & $-1{,}8$ \\
2 (FZP)          & 55,1 & 58,3 & $-3{,}3$ & $-2{,}4$ \\
3 (płaska)       & 53,8 & 53,6 & $+0{,}1$ & $+1{,}3$ \\
13.06 (płaska, nad łóżkiem) & 52,5 & 52,7 & $-0{,}1$ & $+0{,}4$ \\
\bottomrule
\end{tabular}%
}
\end{table}
...
krawędzi łóżka. Co ciekawe, noc z~płaską pokrywą wypadła najlepiej, a~obie
noce z~soczewką zaniżały tętno. Nie dowodzi to wyższości płaskiej pokrywy,
...
wykazany związek przyczynowy. Pełna noc z~13~czerwca, już w~ustawieniu nad
łóżkiem, również miała praktycznie zerowe średnie obciążenie HR względem
Garmina. Dla nocy z~15 i~16~czerwca eksport zegarka nie był dostępny, dlatego
...
\midrule
1 (plano-convex) & 13,6 & 14,0 & $-0{,}4$ & $-0{,}2$ \\
2 (FZP)          & 14,6 & 14,2 & $+0{,}4$ & $+0{,}5$ \\
3 (płaska)       & 14,3 & 14,0 & $+0{,}3$ & $+0{,}2$ \\
13.06 (płaska, nad łóżkiem) & 14,5 & 13,7 & $+0{,}7$ & $+0{,}6$ \\
\bottomrule
\end{tabular}%
}
\end{table}

Zgodność częstości oddechu okazała się lepsza niż zgodność tętna --- średnie
obciążenie w~pierwszych trzech nocach nie przekraczało pół oddechu na minutę,
a~w~pełnej nocy nad łóżkiem wyniosło około 0,7~oddechu/min. Jest to
najbardziej stabilny wynik badania nocnego, choć nadal stanowi porównanie
z~urządzeniem konsumenckim, a~nie walidację kliniczną.
...
Różnica ,,radar $-$ Garmin'' mierzy więc rozjazd dwóch przybliżeń.
...
Noc & Źródło & Czuwanie & Sen lekki & Sen głęboki & REM \\
 & & [min] & [min] & [min] & [min] \\
\midrule
1 & Radar    & 24,0 & 224,5 & 70,5 & 69,5 \\
1 & Garmin   & 40,0 & 222,0 & 57,0 & 47,0 \\
1 & Różnica  & $-16{,}0$ & $+2{,}5$ & $+13{,}5$ & $+22{,}5$ \\
\midrule
2 & Radar    & 18,0 & 207,0 & 50,0 & 85,5 \\
2 & Garmin   & 5,0  & 211,0 & 18,0 & 117,0 \\
2 & Różnica  & $+13{,}0$ & $-4{,}0$ & $+32{,}0$ & $-31{,}5$ \\
\bottomrule
\end{tabular}
\end{table}

Względem strumienia FIT radar przypisywał więcej czasu fazie snu głębokiego
(o~13,5 i~32~minuty). Wniosek ten trzeba jednak natychmiast osłabić: jak
...
Pierwsza wersja klasyfikatora wykazała w~obu nocach po dwa bezpośrednie
przejścia REM~$\rightarrow$~sen głęboki, podczas gdy Garmin nie wykazał
żadnego. Zamiast dostrajać progi do zegarka, wprowadzono opisany wcześniej,
niezależny od Garmina warunek przejścia przez sen lekki. W~wyniku końcowym
bezpośrednich przejść REM~$\leftrightarrow$~sen głęboki już nie ma; w~nocy~1
przeniesiono do snu lekkiego łącznie 12~minut, a~w~nocy~2 --- 16~minut.
Jest to naprawa niespójności reguł, nie dowód poprawności pozostałych etykiet.

\begin{figure}[p]
    \centering
    \includegraphics[width=\textwidth]{sen_krawedz_fazy.png}
    \caption[Fazy snu w pierwszych trzech nocach]{Estymowane fazy snu
    w~pierwszych trzech nagraniach. Linia fioletowa oznacza wynik A121,
    przerywana --- estymatę Garmina, a~jasne żółte pola --- okna, w~których
    reguła strukturalna wstawiła sen lekki między REM i~snem głębokim. Nocy~3
    nie należy porównywać czasowo z~pozostałymi, ponieważ zapis został
    przerwany.}
    \label{fig:sen-krawedz-fazy}
\end{figure}

\begin{figure}[p]
    \centering
    \includegraphics[width=\textwidth]{sen_krawedz_tetno.png}
    \caption[Tętno w pierwszych trzech nocach]{Tętno w~pierwszych trzech
    nocach: A121 i~Garmin Fenix~7. Linia A121 łączy wyłącznie ważne okna oraz
    krótkie przerwy ruchowe; długie luki pozostawiono celowo, aby nie
    sugerować ciągłości pomiaru tam, gdzie estymata została odrzucona.}
    \label{fig:sen-krawedz-hr}
\end{figure}

\begin{figure}[p]
    \centering
    \includegraphics[width=\textwidth]{sen_krawedz_oddech.png}
    \caption[Częstość oddechu w pierwszych trzech nocach]{Częstość oddechu
    w~pierwszych trzech nocach. Zgodność trendów jest wyraźnie lepsza niż dla
    tętna, zwłaszcza w~spokojnych odcinkach nocy. Widoczne krótkie odchylenia
    i~luki należy interpretować razem z~odrzucaniem okien po ruchu, a~nie jako
    pewne zmiany fizjologiczne.}
    \label{fig:sen-krawedz-rr}
\end{figure}
...
\begin{figure}[p]
    ...
    pokrywą i~radarem na wysięgniku nad łóżkiem. Dla 13~czerwca dostępna jest
...
\begin{figure}[p]
    ...
    81--85\% okien. Dla 13~czerwca trend A121 można bezpośrednio zestawić
...
\begin{figure}[p]
    ...
    tylko dla 13~czerwca zachował się równoległy eksport Garmina.}
    \label{fig:sen-flat-rr}
\end{figure}

\section{Pułapka interpretacyjna: amplituda dominującego odbicia}
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
Noc & Mediana ampl. & Mediana odl. & Mediana odl. & Udział okien \\
 & piku [j.u.] & celu [m] & piku [m] & $|\Delta d|\leq$\SI{0.10}{m} \\
\midrule
1 & 1665 & 0,37 & 0,15 & 0,316 \\
2 & 17516 & 0,44 & 0,12 & 0,012 \\
3 & 23574 & 0,22 & 0,10 & 0,384 \\
\bottomrule
\end{tabular}
\end{table}

Surowy wykres amplitud (rys.~\ref{fig:amplitudy}) sugeruje dramatyczną
poprawę między nocami. Tabela~\ref{tab:amplitudy} pokazuje jednak, dlaczego
nie wolno tego czytać jako „lepszego sygnału z~klatki piersiowej”:
w~nocy~2 jedynie \textbf{1,2\%} okien miało dominujący pik w~promieniu
\SI{10}{cm} od śledzonego celu. Silne odbicie pochodziło niemal na pewno
od krawędzi łóżka lub innego bliskiego reflektora, a~nie od badanego.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{sen_amplitudy_nocy.png}
    \caption[Amplituda dominującego odbicia w trzech nocach]{Amplituda
    dominującego odbicia w~trzech nocach. Wykres jest użyteczny jako kontekst
    inżynierski, ale nie stanowi miary siły sygnału z~klatki piersiowej.}
    \label{fig:amplitudy}
\end{figure}
...
tułowia. Taki mechanizm jest zgodny zarówno z~najlepszą zgodnością tętna
w~nocy~3, jak i~z~ciągłymi przebiegami trzech późniejszych nocy nad łóżkiem.
Nie wolno jednak przypisać całej poprawy samej pokrywie: jednocześnie zmieniono
...
hiperboliczną przy medianie \SI{1.62}{m} pozostaje świadomie wyłączona z~tych
wniosków, ponieważ ponad \SI{60}{\degree} odchylenia od normalnej czyniło
geometrię nieporównywalną.

\todo{Powtórzyć pełną noc z~soczewką hiperboliczną z~większego dystansu,
ustawiając oś możliwie prostopadle do klatki i~dokumentując dystans oraz kąt.
Porównać ją z~płaską pokrywą przy tym samym mocowaniu nad łóżkiem; nie używać
nieudanego nagrania ukośnego jako wyniku ujemnego soczewki.}
...
\begin{itemize}
  ...
  \item Na dystansie \SIrange{0.5}{1.0}{m} folia aluminiowa nie zwiększyła
  mierzalnie amplitudy odbicia. Z~powodu wykonania warunków w~osobnych sesjach
  wpływ folii na SNR pozostaje nierozstrzygnięty i~wymaga krótkiego,
  przeplatanego retestu przy \SI{2}{m}.
  \item W~pomiarach nocnych zgodność częstości oddechu z~Garmin~Fenix~7
  mieści się w~granicach $\pm 0{,}5$~oddechu na minutę, a~tętna w~granicach
  kilku uderzeń na minutę. Rozbieżności w~fazach snu wskazują na ograniczenia
  heurystycznego klasyfikatora --- co jest bezpośrednią motywacją dla
